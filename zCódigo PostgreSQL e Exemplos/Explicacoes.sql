...
-- funcionario: Complementa as informações de um usuario que é do tipo 'funcionario', adicionando detalhes específicos da função (cargo, código do funcionário, supervisor, etc.).
-- Esta separação é interessante porque um usuario pode ser um funcionário hoje e amanhã não ser mais (ou se tornar um cliente), sem perder seu registro de usuário.

-- Como funciona a hierarquia com seu esquema atual:

-- A hierarquia é primariamente definida pelo campo cargo na tabela funcionario e pela auto-referência id_supervisor. A coluna tipo_usuario na tabela usuario serve para uma categorização mais ampla de usuario (cliente ou funcionário), o que é útil para controle de acesso em um nível mais geral na aplicação.
...
-- funcionario: Acessa funcionalidades administrativas (gerenciar contas, aprovar empréstimos, etc.).
-- Hierarquia de permissões dentro do papel funcionario (via funcionario.cargo):

-- Estagiário: Permissões mais limitadas (e.g., visualização, entrada de dados básica).
-- Gerente: Permissões intermediárias (e.g., aprovar algumas operações, visualizar relatórios completos).
-- Admin: Permissões mais amplas (e.g., aprovar tudo, gerenciar usuários, configurar sistema).
...
-- Você verifica o usuario.tipo_usuario. Se for 'funcionario', você continua.
