...
-- Função para criar conta ao aprovar solicitação
...
BEGIN
  -- Só cria conta se status mudou para 'APROVADO'
  IF NEW.status = 'APROVADO' AND (OLD.status IS DISTINCT FROM 'APROVADO') THEN
    -- Gerar número de conta simples: 'AC' + id_solicitacao + timestamp (ou crie sua regra)
    ...
    VALUES (
      v_numero_conta,
      1, -- coloque o id_agencia padrão, ou ajuste conforme sua regra
      0.00,
      NEW.tipo_conta,
      NEW.id_cliente,
      CURRENT_DATE,
    ...
    -- Atualiza data_aprovacao e id_funcionario_aprovador se necessário
    -- (Pode ser feito no front/back, ou no UPDATE que disparou trigger)
  END IF;
...
-- Função para registrar ações na tabela auditoria
